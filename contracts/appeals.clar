...
;; Implements appeal bonds, limits, and proper error handling
...
(define-constant APPEAL_QUORUM_PERCENTAGE u5) ;; 5% of total supply needed for appeal quorum (increased from 1%)
(define-constant APPEAL_BOND_AMOUNT u5000000) ;; Appeal bond amount (5 tokens with 6 decimals) - Phase 2
(define-constant MAX_APPEALS_PER_USER u5) ;; Maximum appeals per user - Phase 2
...
;; Appeal tracking for limits (Phase 2)
(define-map user-appeal-count principal uint)

;; Appeal outcomes tracking (Phase 2)
...
(define-public (create-appeal (content-id uint) (reason (string-ascii 500)) (evidence (string-ascii 1000)))
  (let ((appeal-id (+ (var-get appeal-counter) u1))
        ...
        (user-appeals (default-to u0 (map-get? user-appeal-count tx-sender))))
    (begin
      ;; Get appellant balance with proper error handling
      (let ((appellant-balance (unwrap! (contract-call? .governance-token get-balance tx-sender) ERR_INSUFFICIENT_TOKENS)))
        (begin
          ;; Check appeal limit (Phase 2)
          (asserts! (< user-appeals MAX_APPEALS_PER_USER) ERR_INSUFFICIENT_TOKENS)

          ;; Check if appellant has minimum tokens plus bond (Phase 2)
          (asserts! (>= appellant-balance (+ MIN_APPEAL_TOKENS APPEAL_BOND_AMOUNT)) ERR_INSUFFICIENT_TOKENS)
          ...
          ;; Update appeal count for user (Phase 2)
          (map-set user-appeal-count tx-sender (+ user-appeals u1))
...
;; Get user appeal count (Phase 2)
(define-read-only (get-user-appeal-count (user principal))
  (default-to u0 (map-get? user-appeal-count user))
)

;; Get appeal outcome (Phase 2)
(define-read-only (get-appeal-outcome (appeal-id uint))
  (map-get? appeal-outcomes {appeal-id: appeal-id})
)

;; Record appeal outcome (Phase 2)
(define-public (record-appeal-outcome (appeal-id uint) (upheld bool))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_UNAUTHORIZED)
    (map-set appeal-outcomes
      {appeal-id: appeal-id}
      {
        upheld: upheld,
        bond-returned: false,
...
(define-read-only (get-appeal-counter)
  (var-get appeal-counter)
)

;; Analytics: Get total appeals (Phase 4)
(define-read-only (get-total-appeals)
  (var-get appeal-counter)
)

;; Analytics: Get appeal statistics (Phase 4)
(define-read-only (get-appeal-stats)
  {
    total-appeals: (var-get appeal-counter),
    appeal-quorum-percentage: APPEAL_QUORUM_PERCENTAGE,
    appeal-voting-period: APPEAL_VOTING_PERIOD,
    appeal-bond-amount: APPEAL_BOND_AMOUNT,
    max-appeals-per-user: MAX_APPEALS_PER_USER
  }
)
